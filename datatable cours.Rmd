...
of rows
 ...
 + Si on veut mettre des NA : *:= NA_real_*
...
+ Avec une fct : *data.table::merge(x, y, [options])* (comme les join tidy, voir les options)
...
+ Rolling join
+ Combine rows of 2 datatables : *rbind(dt_a, dt_b)*
+ Combine columns of 2 datatables : *cbind(dt_a, dt_b)*
...
+ Long to wide format : *dcast(dt, id ~ y, value.var = c("a", "b"), fun.aggregate = list(mean, sum))*
with:
...
+ *%like% "^x"* :	Reconnaissance d’une chaîne de caractères
...
de fichiers plats (= fichiers texte)
...
modif_variable <- function(dt, variable = "x"){
  dt_copy <- data.table::copy(dt) # une copie pour la sécurité
