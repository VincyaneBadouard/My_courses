...
+ par ordre croissant : **sort(x, decreasing = FALSE, na.last = NA, ...)** ou **order()**
...
+ Joindre 2 tables :
```{r}
left_join(df1, df2, by = 'key') %>% # by = c('coldf1' = 'coldf2')) if not the same name
  mutate(
...
+ temps par ordre croissant **data <- data[order(data$Time),]**
